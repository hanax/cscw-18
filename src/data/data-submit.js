...
To bridge the gap until the 2018 paper deadline, we offered an Online First call for paper participation. The Online First deadline has passed, but details about this process can be found [here](./archived.html).
`
    }
  ]
};

const submitPapersContents = {
  items: [
    {
...
`
    ...
    {
      title: "Workshops Co-Chairs",
      content: `
Chiara Rossitto (Stockholm University)
Brian Keegan (University of Colorado)
[workshops2018@cscw.acm.org](mailto:workshops2018@cscw.acm.org)
`
    }
  ]
};
...
[posters2018@cscw.acm.org](mailto:posters2018@cscw.acm.org)
`
    }
  ]
};

const submitPanelsContents = {
  items: [
    {
      title: "Important Dates",
      content: `
...
All questions should be directed to [dc2018@cscw.acm.org](mailto:dc2018@cscw.acm.org).
...
[dc2018@cscw.acm.org](mailto:dc2018@cscw.acm.org)
`
    }
  ]
};

const submitDemosContents = {
  items: [
    {
      title: "Important Dates",
      content: `
...
`
    ...
    {
      title: "About Demonstrations",
      content: `
CSCW 2018 demonstrations present implementations of new CSCW systems and concepts. The curated demonstrations allow conference participants to view novel and noteworthy CSCW systems in action, discuss the systems with those who created them, and try them out. Appropriate demonstrations include applications, technologies, and research prototypes, and may showcase work that has been or is being published at CSCW or elsewhere. Demonstrations can also serve to showcase novel commercial products not previously described in the research literature. 
...
* State whether this is intended as a reception or integrated demo.

* Information about the presenter(s), including their relationship to the project.

* Demonstration requirements. Provide a detailed description of what you need to make the demonstration work, such as needed space, equipment support, and Internet connectivity. If your submission is an integrated demo, please also describe in detail the logistics of how the conference attendees will get access to and interact with the system throughout the conference, and what added value attendees will receive from this special style of presentation.
...
`
    ...
    {
      title: "Demonstrations Co-Chairs",
      content: `
Malte Jung (Cornell University),
Naomi Yamashita (NTT Communication Science Labs, Japan)
[demos2018@cscw.acm.org](mailto:demos2018@cscw.acm.org)
`
    }
  ]
};

const submitTravelContents = {
  items: [
    {
...
Please use the SIGCHI papers template to format your submission (there is a [Word version](https://www.acm.org/publications/authors/submissions) and a [Latex version](https://www.acm.org/publications/authors/submissions)). We ask that you use the templates provided here and not the proceedings template provided on the ACM official site. Papers should be converted to PDF before submission.
...
submitOverviewContents.items = submitOverviewContents.items.map((n) => {
  n.content = marked(n.content);
  return n;
});

submitPapersContents.items = submitPapersContents.items.map((n) => {
  n.content = marked(n.content);
  return n;
});

submitWorkshopsContents.items = submitWorkshopsContents.items.map((n) => {
  n.content = marked(n.content);
  return n;
});

submitPostersContents.items = submitPostersContents.items.map((n) => {
  n.content = marked(n.content);
  return n;
});

submitPanelsContents.items = submitPanelsContents.items.map((n) => {
  n.content = marked(n.content);
  return n;
});

submitDCContents.items = submitDCContents.items.map((n) => {
  n.content = marked(n.content);
  return n;
});

submitDemosContents.items = submitDemosContents.items.map((n) => {
  n.content = marked(n.content);
  return n;
});

submitTravelContents.items = submitTravelContents.items.map((n) => {
  n.content = marked(n.content);
  return n;
});

submitArchivedContents.items = submitArchivedContents.items.map((n) => {
  n.content = marked(n.content);
  return n;
});
